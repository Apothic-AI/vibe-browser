use crate::ai::{
    pocketflow::{GeneratorNode, PocketFlowOrchestrator, RequirementsNode, ValidationNode},
    providers::{AIProvider, AIProviderFactory},
...
impl AIState {
    ...
    pub async fn initialize_orchestrator(&mut self) -> Result<()> {
        if let Some(provider_config) = self.config_manager.get_active_provider().await? {
            ...
            let orchestrator =
                PocketFlowOrchestrator::new(requirements_node, generator_node, validation_node);
...
pub async fn generate_component(
    ...
    log::info!(
        "Generating component for requirements: {}",
    ...
    {
        log::info!(
            "Returning cached component: {}",
            cached_response.component_name
        );
...
pub async fn stream_component_generation(
    ...
    log::info!(
        "Starting streaming component generation for session: {}",
        session_id
    );
    ...
    if state.orchestrator.is_none() {
        ...
        return Ok(CommandResponse::error(
            "AI provider not configured".to_string(),
        ));
    }
    ...
    tokio::spawn(async move {
        match orchestrator
            .generate_component_streaming(request.clone(), callback)
            .await
        {
            Ok(response) => {
                // Cache the successful response
                ...
                log::info!(
                    "Streaming component generation completed for session: {}",
                    session_id
                );
            }
            Err(e) => {
                log::error!(
                    "Streaming component generation failed for session {}: {}",
                    session_id,
                    e
                );
            }
...
pub async fn validate_component(
    ...
    log::info!(
        "Validating component code (length: {} chars)",
        component_code.len()
    );
    ...
    inputs.insert(
        ...
        serde_json::to_value(&mock_response).unwrap(),
    );

    match validation_node.execute(inputs).await {
        Ok(outputs) => {
            if let Some(response_value) = outputs.get("response") {
                if let Ok(validated_response) =
                    serde_json::from_value::<ComponentGenerationResponse>(response_value.clone())
                {
                    let result = ValidationResult {
                        is_valid: matches!(
                            validated_response.validation_status,
                            crate::ai::ValidationStatus::Valid
                        ),
            ...
            Ok(CommandResponse::error(
                "Failed to parse validation result".to_string(),
            ))
        }
        Err(e) => {
...
pub async fn configure_ai_provider(
    ...
    if !state
        .config_manager
        .validate_ai_provider(&config)
        .await
        .unwrap_or(false)
    {
        return Ok(CommandResponse::error(
            "Invalid AI provider configuration".to_string(),
        ));
    }
    ...
    match state.config_manager.store_ai_provider(config, &name).await {
        Ok(_) => {
            ...
            match state.config_manager.get_ai_provider(&name).await {
                    ...
                    match AIProviderFactory::create_provider(stored_config) {
                        Ok(provider) => {
                            ...
                            match provider.validate_config().await {
                                Ok(_) => {
                                    log::info!(
                                        "AI provider {} configured and validated successfully",
                                        name
                                    );
                ...
                Ok(None) => Ok(CommandResponse::error(
                    "Failed to retrieve stored configuration".to_string(),
                )),
                Err(e) => {
        ...
        Ok(true) => {
            ...
            if let Err(e) = state_mut.initialize_orchestrator().await {
                ...
                return Ok(CommandResponse::error(format!(
                    "Failed to initialize with new provider: {}",
                    e
                )));
            }
            ...
            Ok(CommandResponse::success(
                "Active provider updated".to_string(),
            ))
        }
        Ok(false) => Ok(CommandResponse::error("Provider not found".to_string())),
        Err(e) => {
...
pub async fn get_cached_components(
    ...
    match state.cache.list_cached_components(limit, offset).await {
        ...
        Err(e) => {
            ...
            Ok(CommandResponse::error(format!(
                "Failed to get cached components: {}",
                e
            )))
        }
    }
